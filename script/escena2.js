...
function setupScene() {
  ...
  zombielistener = new THREE.AudioListener();
  zombieaudioLoader = new THREE.AudioLoader();
  zombieSound = new THREE.Audio(listener2);
  
  zombieaudioLoader.load('audio/Zombie Growl 5.wav', function (xd) {
    zombieSound.setBuffer(xd);
...
function render() {
  ...
  if (twice == true && (purpleValidator2 == false ||
    ...
    || redValidator2 == false) && !isPaused) {

    personajePrincipal = scene.getObjectByName("player1");
    zombie = scene.getObjectByName("zombie");
    personajePrincipal.rotation.y += yaw * deltaTime;
    personajePrincipal.translateZ(forward * deltaTime);
    var modalperdido = document.getElementById('modal-perdiste');
    var btnsalir = document.getElementById('btn-salir');


      var xd = new THREE.Vector3(personajePrincipal.position.x,personajePrincipal.position.y,personajePrincipal.position.z);
      var yd = new THREE.Vector3(zombie.position.x,zombie.position.y,zombie.position.z);
      var dicks = zombie.position.distanceTo(personajePrincipal.position);
      ...
      if(isPlayerClose){
        ...
        zombie.position.y += .0004*200*(subvec.y/hypotenuse);
        zombie.position.x += .0004*200*(subvec.x/hypotenuse);
        zombie.position.z += .0004*200*(subvec.z/hypotenuse);
        zombie.rotation.y = personajePrincipal.rotation.y+subvec.y;
      }

      if(hypotenuse < 1){
        modalperdido.style.display = 'block';
  ...
  if (purpleValidator2 == true &&
    ...
    personajePrincipal.jump.stop();
    personajePrincipal.run.stop();
    var victory = document.getElementById("modal-ganaste");
  ...
  var next = document.getElementById("btn-nextlevel");
  var salir = document.getElementById("btn-multi");
  next.onclick = function(){
    window.location.replace("scene3game.html");
...
function cargar_objetos() {
  ...
  //Enemigo
  enemigo = new THREE.FBXLoader();
  enemigo.load('resources/badguy/Prisoner B Styperek.fbx', function (personaje) {
    personaje.position.y = 0.5;      //altura
    ...
    const anim1 = new THREE.FBXLoader();
    anim1.load('resources/badguy/Orc Idle.fbx', (anim) => {
      var animation = new THREE.AnimationMixer(personaje);
      personaje.idle = animation.clipAction(anim.animations[0]);
      personaje.idle.play();
      mixers.push(animation);
    ...
    const anim2 = new THREE.FBXLoader();
    anim2.load('resources/badguy/Injured Run.fbx', (anim) => {
      var animation = new THREE.AnimationMixer(personaje);
      personaje.run = animation.clipAction(anim.animations[0]);
      mixers.push(animation);
    });
